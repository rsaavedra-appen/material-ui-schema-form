...
var stdFormObj = function (name, schema, options) {
  ...
  if (schema.style) {
    f.style = schema.style;
  }
  
  if (schema.htmlClass) {
    f.htmlClass = schema.htmlClass;
  }
  ...
  f.schema = schema; // Ng model options doesn't play nice with undefined, might be defined
  // globally though
...
var fieldset = function (name, schema, options) {
  if (stripNullType(schema.type) === 'object') {
    ...
    f.type = 'fieldset';
    f.items = [];
    options.lookup[ObjectPath.stringify(options.path)] = f; //recurse down into properties
...
var array = function array(name, schema, options) {
  if (stripNullType(schema.type) === 'array') {
    ...
    f.type = 'array';
    f.key = options.path;
    options.lookup[ObjectPath.stringify(options.path)] = f; // don't do anything if items is not defined.
...
function defaultFormDefinition(name, schema, options) {
  ...
  var rules = defaults[stripNullType(schema.type)]; //console.log('defaultFormDefinition:defaults = ', defaults);
...
function getDefaults(schema, ignore, globalOptions) {
  ...
  ignore = ignore || {};
  globalOptions = globalOptions || {}; //console.log('getDefaults:schema.type = ', schema.type);
  ...
  if (stripNullType(schema.type) === 'object') {
    ...
    for (var k in schema.properties) {
      if (schema.properties.hasOwnProperty(k)) {
        if (ignore[k] !== true) {
          var required = schema.required && schema.required.indexOf(k) !== -1; //console.log('getDefaults:required = ', required);
...
function merge(schema, form, ignore, options, readonly) {
  ...
  var stdForm = getDefaults(schema, ignore, options); //console.log('merge stdForm', stdForm);
  ...
  var lookup = stdForm.lookup; //console.log('form', form);
